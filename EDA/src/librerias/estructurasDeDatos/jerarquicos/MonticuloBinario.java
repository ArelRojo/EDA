...
public class MonticuloBinario<E extends Comparable<E>> implements ColaPrioridad<E> {
	...
	public void insertar(E e) {
		...
		/*
		 * while (posIns > 1 && e.compareTo(elArray[posIns / 2]) < 0) { elArray[posIns]
		 * = elArray[posIns / 2]; posIns = posIns / 2; }
		 */
	...
	/**
	 * obtiene un String con los datos de una CP ordenados Por Niveles y con el
	 * formato que se usa en el estandar de Java (entre corchetes cuadrados y
	 * separando cada elemento del anterior mediante una coma seguida de un espacio
	 * en blanco); si la CP esta vacia el String resultado es []
	 */
	...
	public int menoresQue(E e) {
		int res = 0;
		for(int i = 1; i <= talla; i++) {
			if(elArray[i].compareTo(e) < 0) {res++;}
		}
		return res;
	}
