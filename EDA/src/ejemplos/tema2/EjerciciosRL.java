...
public class EjerciciosRL {
    ...
    /** Sea v un array de enteros que se ajustan 
    ...
    private static int puntoCruce(int[] v, int i, int j) {
         ...
         */
        int m = (i + j) / 2;
        /** Para que m sea el punto de cruce (m = k), como minimo: v[m] <= 0 */
        if (v[m] <= 0) {
            /** Si, ademas, v[m + 1] > 0, entonces m es el punto de cruce */
            if (v[m + 1] > 0) return m; 
    ...
    /** Sea v un array de enteros positivos que se ajustan 
    ...
    /** Busqueda de la componente de un array con valor igual a posicion. 
     * 
    ...
    /** Busqueda de dos String en posiciones consecutivas de un array.
     * 
    ...
    public static boolean vecinas(String[] v, String x, String y, int i, int j) {
        if (i >= j) return false;
        int m = (i + j) / 2;
